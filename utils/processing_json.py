...
def preprocess_json(json_file: dict) -> dict:
    ...
    for data in input_data:
        text_data.append(data['name'] + " " + " ".join(data['props']).replace("\t", " "))
...
def postprocess_json(data, result):
    response = [{"id":product_id, "reference_id":model2client(model_id)} for product_id, model_id in zip(data['id'], result)]
    return response